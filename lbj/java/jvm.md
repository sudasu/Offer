...
此部分内存空间并不属于jvm规范中定义的区域，但会经常被使用并可能导致OutOfMemoryError(直接内存过大占用总内存，导致jvm内存分配不够)。在JDK1.4中加入的NIO类，通过Channel和Buffer之间的I/O方式，使用Native函数直接分配对外内存，通过DirectByteBuffer对象对这块内存进行引用，避免Java堆和Native堆之间来回复制数据。(零拷贝与此类似的，本来从A拷贝到B，期间主要是需要从磁盘或者其他设备先拷贝到内核缓冲区，再从内核拷贝到用户缓冲区然后再开始写操作，后面还要走一遍内核缓冲区到用户缓冲区的写流程。而零拷贝可以省略内核缓冲区到用户缓冲区这一步，直接从内核缓冲区去读，读完后写到如socket缓冲区之类的写缓冲区。从原本的5此拷贝，4次上下文切换(读写各两次)，变成3次拷贝，2次上下文切换(sendfile两次)。此外零拷贝还能从硬件级别继续优化，即在内核缓冲区拷贝后，将内核缓冲区的描述信息如大小，位移信息拷贝至socket缓冲区，这样就可以继续减少一次拷贝次数，且全程没有CPU参与只有DMA参与。)
